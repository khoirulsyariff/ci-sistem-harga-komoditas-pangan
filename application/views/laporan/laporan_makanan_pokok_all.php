...
if (isset($_GET['tanggal'])) {
		?>
            <div class="box-body table-responsive">
				Tanggal : <?php $tanggal_new = date_create($tanggal); echo date_format($tanggal_new,'d')." ".bulan(date_format($tanggal_new,'m'))." ".date_format($tanggal_new, 'Y'); ?>
				<div class="pull-right">Pasar: Semua Pasar </div>
...
if(empty($harga)){ $harga_sampel1 = 0; $harga_sampel2=0; $harga_sampel3=0; }else{ $harga_sampel1=$harga->h_sampel1; $harga_sampel2 = $harga->h_sampel2; $harga_sampel3 = $harga->h_sampel3; }
$harga_komoditas = ($harga_sampel1 + $harga_sampel2 + $harga_sampel3)/3;
if($harga_komoditas == 0){$harga_komoditas_all = 0; }else{ $harga_komoditas_all = $harga_komoditas/$jumlah_pasar; }
$harga_kemarin = $this->m_laporan_makanan_pokok->harga_komoditas_all($tanggal_kemarin, $data_pangan->id_pangan)->row();
if(empty($harga_kemarin)){ $harga_sampel1_kemarin = 0; $harga_sampel2_kemarin=0; $harga_sampel3_kemarin=0; }else{ $harga_sampel1_kemarin=$harga_kemarin->h_sampel1; $harga_sampel2_kemarin = $harga_kemarin->h_sampel2; $harga_sampel3_kemarin = $harga_kemarin->h_sampel3; }
$harga_komoditas_kemarin = ($harga_sampel1_kemarin + $harga_sampel2_kemarin + $harga_sampel3_kemarin)/3;
if($harga_komoditas_kemarin == 0){$harga_komoditas_all_kemarin = 0; }else{ $harga_komoditas_all_kemarin = $harga_komoditas_kemarin/$jumlah_pasar; }
...
?>
				<tr>
					...
					<td><?php echo number_format($harga_komoditas_all, 2, ',', '.'); ?></td>
...
if ($this->session->userdata('level') == 'admin') {
				?>
                  <a target="_blank" href="<?php echo base_url('laporan/cetak-makanan-pokok-all?tanggal=' . $_GET['tanggal'].'&pasar='. $_GET['pasar']); ?>" target="_blank">
                    <button class="btn btn-warning btn-custom-cetak-export"><i class="fa fa-print"></i> <span>Cetak</span></button>
                  </a>
                  <a href="<?php echo base_url('grafik/grafik-makanan-pokok-all?tanggal=' . $_GET['tanggal'].'&pasar='. $_GET['pasar']); ?>" target="_blank">
